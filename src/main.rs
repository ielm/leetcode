...
async fn main() {
    ...
    loop {
        ...
        let problem = fetcher::get_problem(_id).await.unwrap_or_else(|| {
            panic!(
...
async fn deal_solving(id: &u32) {
    ...
    let mut lib_file = fs::OpenOptions::new()
        ...
        .unwrap();
    writeln!(lib_file, "mod {};", solution_name).expect("Could not add mod");
}
...
fn deal_problem(problem: &Problem, code: &CodeDefinition, write_mod_file: bool) {
    ...
    if write_mod_file {
        let mut lib_file = fs::OpenOptions::new()
            ...
            .open("./src/problem/mod.rs")
            .unwrap();
        writeln!(lib_file, "mod {};", file_name).expect("Could not add mod");
    }
}
